...
const teachingLexiconPath = join(__dirname, "data/teaching-lexicon.json");
const elisionBridgesPath = join(__dirname, "data/elision-bridges.json");
const elisionBridgesOutPath = join(
  root,
  "src/lib/data/elisionBridges.generated.ts",
);
...
async function main() {
   ...
   * @returns {stress is (0 | 1 | 2)[]}
   */
  function isValidStressCodes(stress, syllableCount) {
    if (!Array.isArray(stress) || stress.length !== syllableCount) return false;
    return stress.every(
      (code) => Number.isInteger(code) && code >= 0 && code <= 2,
    );
  }

  // Teaching / classroom lemmas (insert or override citation primary).
  /** @type {Record<string, { syllables: number, stress: number[] }>} */
  const teaching = JSON.parse(readFileSync(teachingLexiconPath, "utf8"));
  let teachingInjected = 0;
  let teachingOverridden = 0;
  for (const [rawWord, entry] of Object.entries(teaching)) {
    const word = normalizeLemma(rawWord);
    if (
      !word ||
      !entry ||
      typeof entry.syllables !== "number" ||
      entry.syllables < 1 ||
      entry.syllables > STRESS_PACK_MAX_SYLLABLES ||
      !isValidStressCodes(entry.stress, entry.syllables)
    ) {
      throw new Error(`invalid teaching lexicon entry for "${rawWord}"`);
    }
    const packed = packStressPattern(
      /** @type {(0 | 1 | 2)[]} */ (entry.stress),
    );
    if (word in stressPacked) {
      const oldSyl = syllables[word];
      const oldPacked = stressPacked[word];
      if (oldSyl !== entry.syllables || oldPacked !== packed) {
        // Preserve citation primary as a variant when teaching replaces it.
        if (oldSyl !== undefined && oldPacked !== undefined) {
          const existing = variantsByWord[word] ?? [];
          const key = `${oldSyl}:${oldPacked}`;
          if (!existing.some((a) => `${a.syllables}:${a.packedStress}` === key)) {
            existing.push({ syllables: oldSyl, packedStress: oldPacked });
            variantsByWord[word] = existing;
          }
        }
        syllables[word] = entry.syllables;
        stressPacked[word] = packed;
        teachingOverridden += 1;
      }
    } else {
      syllables[word] = entry.syllables;
      stressPacked[word] = packed;
      // Ensure lexicon inclusion (writePronunciationPacks keys off byWord).
      if (!(word in byWord)) byWord[word] = [];
      teachingInjected += 1;
    }
  }
  ...
  for (const [rawWord, alts] of Object.entries(poetic)) {
    ...
    for (const alt of alts) {
      if (
        ...
        !isValidStressCodes(alt.stress, alt.syllables)
      ) {
  ...
  // Runtime elision bridges (apostrophe form → citation lemma for meter-fit).
  /** @type {Record<string, string>} */
  const elisionBridges = JSON.parse(readFileSync(elisionBridgesPath, "utf8"));
  /** @type {Array<[string, string]>} */
  const bridgeEntries = [];
  const seenBridgeSrc = new Set();
  for (const [from, to] of Object.entries(elisionBridges)) {
    const src = normalizeLemma(from);
    const dest = normalizeLemma(to);
    if (!src || !dest) {
      throw new Error(`invalid elision bridge "${from}" → "${to}"`);
    }
    if (seenBridgeSrc.has(src)) {
      throw new Error(`duplicate elision bridge source "${src}"`);
    }
    if (!(dest in stressPacked)) {
      throw new Error(
        `elision bridge "${src}" → "${dest}" has unknown destination`,
      );
    }
    seenBridgeSrc.add(src);
    if (!(src in stressPacked)) {
      console.warn(
        `elision bridge source "${src}" is not in the lexicon (meter-fit uses bridge alts only)`,
      );
    }
    bridgeEntries.push([src, dest]);
  }
  const bridgeLines = [
  ...
  writeFileSync(elisionBridgesOutPath, bridgeLines.join("\n"));
  ...
  console.log(
    `Elision bridges: ${bridgeEntries.length} → ${elisionBridgesOutPath}`,
  );
  console.log(
