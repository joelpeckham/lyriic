...
export function composeFormToolPage(
  ...
  return {
    ...
    faqs: [
      ...form.faqs,
      {
        q: "Can I continue this draft in the editor?",
        plain: `Yes. Use Continue in the editor with ${entry.label} — if you edited the checker (not just the stock sample), that text carries into the editor once. Nothing is uploaded.`,
        href: writerPath(form.meterId),
        hrefLabel: `Continue in the editor with ${entry.label}`,
      },
    ],
    sampleLines: form.sampleLines,
    cta: `Continue in the editor with ${entry.label}`,
