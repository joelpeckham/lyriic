...
function isLyAdverb(word: string): boolean {
  return word.endsWith("ly") && word.length > 3 && !LY_NON_ADV.has(word);
}

/**
 * Morphology without treating bare -ed/-ing as verbs — those are too often
 * adjectives or nouns when context is missing.
 */
...
export function detectUsage(
  ...
  if (prev && DETERMINERS.has(prev)) {
    if (isLyAdverb(normalized)) return "r";
    const morph = morphologyUsage(normalized);
    if (morph === "a") return "a";
    // Participial adjectives are common after determiners (a tired man).
    if (normalized.endsWith("ed") && normalized.length > 3) return "a";
    return "n";
  }
  if (prev === "to") return "v";
  ...
  if (prev && DEGREE.has(prev)) {
    return isLyAdverb(normalized) ? "r" : "a";
  }
  if (prev && COPULA.has(prev)) {
    // Progressive: she is writing / have been running.
    if (normalized.endsWith("ing") && normalized.length > 4) return "v";
    return isLyAdverb(normalized) ? "r" : "a";
  }
